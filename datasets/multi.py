...
from datasets import affectnet, celeba
...
class MultiFaceDataset(td.Dataset):
    def __init__(self, datasets, train=True, shuffle=True, max_samples=None):
        ...
        # self.joint_idx = sklearn.utils.shuffle(np.hstack(inds).transpose())
...
if __name__ == '__main__':
    ...
    datasets = [
        ...
        # emotiw.EmotiWFaces(train=train, max_samples=1000),
        # voxceleb.VoxCeleb(train=train, max_samples=1000, with_bumps=False)
        celeba.CelebA(train=train, max_samples=1000)
    ]
