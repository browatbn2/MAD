...
class VggFace2(td.Dataset):
    ...
    def __init__(self, root_dir=cfg.VGGFACE2_ROOT, train=True, color=True, start=None,
        ...
        self.cropped_img_dir = os.path.join(root_dir_local, split_subfolder, 'crops', crop_source)
        ...
        if select_subset:
            print("Number of images: {}".format(len(self)))
            self.annotations = self.annotations[self.annotations.of_conf > min_conf]
            print("Number of images: {}".format(len(self)))
            min_rot_deg = 0
            max_rot_deg = 90
...
if __name__ == '__main__':
    ...
    # from utils.nn import to_numpy, GroupedIter, MacroBatch
    # shuffle_macrobatch = False
    # shuffle_microbatch = False
    # ds_iter = GroupedIter(ds.labels, 1000)
    # dl = td.DataLoader(ds_iter, batch_size=8000, shuffle=shuffle_macrobatch, num_workers=0, collate_fn=lambda b: b)
    # mb_data = next(iter(dl))
    # mb = MacroBatch(mb_data, ds)
    # micro_batch_loader = td.DataLoader(mb, batch_size=10, shuffle=shuffle_microbatch, num_workers=0)
